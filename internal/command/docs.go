...
func GenerateDocs(t *Table) []byte {
	...
	b.WriteString("每条命令所需的权限范围（scope）、危险类与 confirm 口径、是否人类专属，以及 REST 映射，都来自 `internal/command` 的命令表；")
	b.WriteString("CLI、REST、MCP 三个投影从同一张表构造。危险类命令须带字符串 `confirm`（object 填对象名、count 填本次数量）；")
	b.WriteString("人类专属命令对任何令牌拒绝、MCP 上不可用。\n\n")
	b.WriteString("## 经主控的命令\n\n")
	b.WriteString("| 命令 | 类别 | scope | 危险类 | confirm | 人类专属 | 列表 | REST |\n")
	b.WriteString("|---|---|---|---|---|---|---|---|\n")
	for _, c := range t.Remote() {
		...
		fmt.Fprintf(&b, "| `%s` | %s | %s | %s | %s | %s | %s | `%s %s` |\n",
			c.Name(), c.Class, info.Scope, dash(string(c.Danger)), dash(confirm), yesNo(c.HumanOnly), yesNo(c.List), info.REST.Method, info.REST.Path)
	}
	b.WriteString("\n## 本地命令（不经主控，只在 CLI 里）\n\n")
	b.WriteString("| 命令 | 说明 |\n|---|---|\n")
